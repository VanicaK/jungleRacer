class Game {
  ...
  start() {
    ...
    this.addSprites(bushes, 4, bushImage, 0.02);

    // Adding coin sprite in the game
    this.addSprites(grasses, 18, grassImage, 0.09);
  }
    ...
    if (allPlayers !== undefined) {
      ...
      for (var plr in allPlayers) {
        ...
        if (index === player.index) {
          ...
          ellipse(player.positionX,height-player.positionY,150);
  ...
  handleBush(index) {
    ...
    animals[index-1].overlap(bushes,function(collector,collected){
      player.bushes=185;
      collected.remove();
    })
  ...
  handleGrass(index) {
    ...
    animals[index-1].overlap(grasses,function(collector,collected){
      player.score+=21;
      player.update();
      collected.remove();
    })
